...
export const UpdateWorkScheduleSchema = z.object({
  compensationId: z.string().min(1, 'Compensation ID is required').optional(),
  organizationId: z.string().min(1, 'Organization ID is required').optional(),
  scheduleType: z.nativeEnum(ScheduleType).optional(),
  defaultStart: z.string().regex(timeRegex, 'Invalid start time format. Use HH:MM').optional(),
  defaultEnd: z.string().regex(timeRegex, 'Invalid end time format. Use HH:MM').optional(),
  workDays: z.array(z.enum(validDays as [string, ...string[]])).optional(),
  restDays: z.array(z.enum(validDays as [string, ...string[]])).optional(),
  overtimeRate: z.number().positive('Overtime rate must be positive').optional(),
  restDayRate: z.number().positive('Rest day rate must be positive').optional(),
  holidayRate: z.number().positive('Holiday rate must be positive').optional(),
  specialHolidayRate: z.number().positive('Special holiday rate must be positive').optional(),
  doubleHolidayRate: z.number().positive('Double holiday rate must be positive').optional(),
  nightShiftStart: z.string().regex(timeRegex, 'Invalid night shift start time').optional(),
  nightShiftEnd: z.string().regex(timeRegex, 'Invalid night shift end time').optional(),
  nightDiffRate: z.number().min(0, 'Night diff rate cannot be negative').optional(),
  rotationPattern: z.string().optional(),
  rotationStart: z.date().optional(),
  shiftGroups: z.array(z.string()).optional(),
  officeDays: z.array(z.enum(validDays as [string, ...string[]])).optional(),
  ...
  coreHoursEnd: z.string().regex(timeRegex, 'Invalid core hours end time').optional(),
  totalHoursPerWeek: z.number().positive('Total hours per week must be positive').optional(),
  isMonthlyRate: z.boolean().optional(),
  monthlyRate: z.number().positive('Monthly rate must be positive').optional(),
  dailyRate: z.number().positive('Daily rate must be positive').optional(),
  hourlyRate: z.number().positive('Hourly rate must be positive').optional(),
  gracePeriodMinutes: z.number().min(0, 'Grace period cannot be negative').optional(),
  requiredWorkMinutes: z.number().positive('Required work minutes must be positive').optional(),
  maxRegularHours: z.number().positive('Max regular hours must be positive').optional(),
  maxOvertimeHours: z.number().positive('Max overtime hours must be positive').optional(),
  allowLateDeduction: z.boolean().optional(),
  maxDeductionPerDay: z.number().positive('Max deduction per day must be positive').optional(),
  maxDeductionPerMonth: z.number().positive('Max deduction per month must be positive').optional(),
  isFlexibleSchedule: z.boolean().optional(),
  minHoursPerDay: z.number().positive('Min hours per day must be positive').optional(),
  maxHoursPerDay: z.number().positive('Max hours per day must be positive').optional(),
  canLogAnyHours: z.boolean().optional(),
});
